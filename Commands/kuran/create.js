...
class createCommand extends Command {
  ...
  async exec(message, { name }) {
    ...
    await data.run(`INSERT INTO Kuran (OwnerID, ServerId, Name, Avatar, Description, Type, MaxMember, Created, Points)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [message.author.id, message.guild.id, name, "", "", 0, -1, Date.now(), 0])
